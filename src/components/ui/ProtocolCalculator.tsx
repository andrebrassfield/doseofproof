...
export function ProtocolCalculator() {
  ...
  return (
    <div className="border border-white/10 rounded-2xl bg-zinc-950 p-6 md:p-8 max-w-xl mx-auto flex flex-col justify-between min-h-[350px]">
      {step === 0 && (
        ...
        <div className="flex-1 flex flex-col justify-between gap-6">
          <div className="space-y-4">
            <div className="flex justify-between items-center text-xs font-mono">
              <span className="text-accent uppercase tracking-widest">Question {step} of {questions.length}</span>
              <span className="text-muted">{Math.round((step / questions.length) * 100)}% Complete</span>
            </div>
            <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden">
              <div className="bg-accent h-full transition-all duration-300" style={{ width: `${(step / questions.length) * 100}%` }} />
            </div>
            <p className="text-lg text-white font-medium leading-relaxed pt-4">
              {currentQ.text}
            </p>
          </div>
      ...
      {step > questions.length && result && (
        <div className="space-y-6 my-auto">
          ...
          <div className="p-4 border border-white/10 rounded-xl bg-black/40">
            ...
            <Link href={`/tests/${result.testSlug}`} className="text-white font-bold hover:text-accent transition-colors flex items-center justify-between text-sm">
