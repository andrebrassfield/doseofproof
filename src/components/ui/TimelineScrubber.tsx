...
interface TimelineEvent {
  ...
  symptoms: string[];
  protocol: string[];
  status: "Exposed" | "Emergency" | "Diagnosed" | "Recovered";
}
...
export function TimelineScrubber() {
  const [activeStep, setActiveStep] = useState(2); // Default to Breakthroughs
  const currentEvent = timelineEvents[activeStep];

  const getStatusColor = (status: string) => {
    switch (status) {
      case "Exposed": return "text-amber-500 border-amber-500/20 bg-amber-500/5";
      case "Emergency": return "text-red-500 border-red-500/20 bg-red-500/5";
      case "Diagnosed": return "text-accent border-accent/20 bg-accent/5";
      case "Recovered": return "text-green-500 border-green-500/20 bg-green-500/5";
      default: return "text-white border-white/10";
    }
  };

  return (
    <div className="border border-white/10 rounded-2xl bg-zinc-950 p-6 md:p-8 flex flex-col gap-8">
      {/* Horizontal Scrubber */}
        ...
        {timelineEvents.map((evt, idx) => (
          <button
            ...
            className={`relative z-10 flex flex-col items-center gap-2 transition-all duration-300 ${
              idx === activeStep ? "scale-110" : "opacity-50 hover:opacity-80"
            }`}
          >
            <div
              className={`w-6 h-6 rounded-full border flex items-center justify-center font-bold font-mono text-[9px] ${
                idx === activeStep ? "bg-accent border-accent text-black" : "bg-zinc-900 border-white/20 text-white"
              }`}
            >
              {idx + 1}
            </div>
      ...
      {/* Detail Display Card */}
      <div className="grid md:grid-cols-[1.1fr_0.9fr] gap-8 border border-white/5 bg-black/40 rounded-xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className={`border px-3 py-0.5 rounded-full text-[9px] font-mono uppercase tracking-wider font-bold ${getStatusColor(currentEvent.status)}`}>
              {currentEvent.status}
            </span>
          </div>
